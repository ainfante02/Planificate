...
class Charts extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->vars($data);
		$this->load->model("Charts_model", "charts_model");
	}
	...
	function index()
	{
		...
		// some data series
		$serie['data'] = $this->charts_model->basic();
	...
	* Output data as array on json string
	...
	function _data()
	{
		$chartData = $this->charts_model->advanced();
		$data['users']['data'] =  $chartData["users"];
		$data['users']['name'] = 'Users by Language';
		$data['popul']['data'] = $chartData["popul"];
		$data['popul']['name'] = 'World Population';
		$data['axis']['categories'] = $chartData["categories"];
		return $data;
	}
